...
Extractor.prototype.extract = function(context) {
	...
	for (var i = 0; i < input.length; i++) {
		...
		if (p === 0) {
			...
			for (var i0 = 0; i0 < self.options.acceptConceptWords.length; i0++) {
				var acceptConceptConcat = self.options.acceptConceptWords[i0];
				if (input.length <= pivot + acceptConceptConcat.length) {
					continue;
				}
				var t = input.substr(pivot, acceptConceptConcat.length).toLowerCase().trim();
				//console.log('t=', t);
				if (acceptConceptConcat === t) {
					p = 5;
					i += acceptConceptConcat.length - 1;
					break;
				}
			}

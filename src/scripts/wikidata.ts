...
export type WikidataQueryResultItem = Dictionary<{ type: string, value: string }>

export async function queryWikidata(query: string): Promise<WikidataQueryResultItem[]> {
    const response = await fetch('https://query.wikidata.org/sparql?format=json&query=' + query, { timeout: 1000 * 20 });
...
export async function queryWikidataInfo(query: string): Promise<Dictionary<WikiEntityInfo>> {
    const entries = await queryWikidata(query);

    const entities: Dictionary<WikiEntityInfo> = {}

    for (const entry of entries) {
        const id = entry['item'].value.substr(entry['item'].value.lastIndexOf('/') + 1);
        const label = entry['label'].value;
        const title = entry['title'].value;
        const alias = entry['alias'].value;
...
export async function getWikiArticleText(lang: string, title: string) {
    ...
    const parts = text.split(/\s*\n\s*/g).filter(item => item && item.length > 10 && !item.trim().startsWith('='));
    return parts.join('\n');
}
...
export async function getWikiEntitiesInfo(lang: string, limit: number) {
    ...
    for (const type of types) {
        let data = await queryWikidataInfo(formatEntitiesQuery(lang, type, limit, 10, false));
        // console.log('got data', data)
        entities = { ...entities, ...data };
        await delay(1000 * 3);
        data = await queryWikidataInfo(formatEntitiesQuery(lang, type, limit, 5, true));
        // console.log('got data', data)
        entities = { ...entities, ...data };
        await delay(1000 * 3);
        // break;
...
function formatEntitiesQuery(lang: string, type: string, limit: number, minSitelinks: number, national: boolean) {
